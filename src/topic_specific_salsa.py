...
def initialize_topic_specific_salsa(nodes_label, num_topic_nodes):
    auths = nodes_label.map(lambda node: (node[0], 0 if node[1] == 0 else 1.0/num_topic_nodes))
    
    hubs = auths
    return auths, hubs

def transition_matrices(edgesDF, spark):
    if (os.path.isdir('../outputs/SALSA/A_transition_matrix/') and os.path.isdir('../outputs/SALSA/A_transition_matrix/')):
        print("Loading transition matrices from file...")
        A = spark.read.parquet("../outputs/SALSA/A_transition_matrix/*").rdd.map(lambda x: (x[0], (x[1][0], x[1][1])))
...
A, H = transition_matrices(edgesDF, spark)

auths, hubs = initialize_topic_specific_salsa(nodes_label, num_topic_nodes)
...
for i in range(num_iter):
    ...
    .join(nodes_label).mapValues(lambda node: (beta*node[0] if node[1] == 0 else beta*node[0] + (1-beta)))
...
hubs.coalesce(1, False).saveAsTextFile("../outputs/SALSA/" + topic_label + "_specific_SALSA_hub_scores.txt")
auths.coalesce(1, False).saveAsTextFile("../outputs/SALSA/" + topic_label + "_specific_SALSA_authority_scores.txt")
