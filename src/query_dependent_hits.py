...
if len(sys.argv) == 1 or len(sys.argv) == 4 or len(sys.argv) > 5:
    print("Usage: spark-submit query_dependent_hits.py topic_label [num_iter] [nodes_csv] [edges_csv]")
...
hubs.coalesce(1, False).saveAsTextFile("../outputs/" + topic_label + "_dependent_hub_scores.txt")
auths.coalesce(1, False).saveAsTextFile("../outputs/" + topic_label + "_dependent_authority_scores.txt")
...
draw_graphs(topic_label + "_dependent_HITS", edges_list, nodes_dict, hubs_dict, auths_dict)