...
if len(sys.argv) == 1 or len(sys.argv) == 5 or len(sys.argv) > 6:
    print("Usage: spark-submit query_dependent_salsa.py topic_label [num_iter] [beta] [nodes_csv] [edges_csv]")
...
for i in range(num_iter):
    ...
    # Hub surfer
    hubs = H.join(hubs).map(lambda x: (x[1][0][0], x[1][0][1]*x[1][1])).reduceByKey(lambda x, y: x + y).mapValues(lambda score: (1-beta) + beta*score)
    
    # Authority surfer
    auths = A.join(auths).map(lambda x: (x[1][0][0], x[1][0][1]*x[1][1])).reduceByKey(lambda x, y: x + y).mapValues(lambda score: (1-beta) + beta*score)
...
hubs.coalesce(1, False).saveAsTextFile("../outputs/SALSA/" + topic_label + "_dependent_teleport_SALSA_hub_scores.txt")
auths.coalesce(1, False).saveAsTextFile("../outputs/SALSA/" + topic_label + "_dependent_teleport_SALSA_authority_scores.txt")
...
draw_graphs(topic_label + "_dependent_teleport_SALSA", edges_list, nodes_dict, hubs_dict, auths_dict)